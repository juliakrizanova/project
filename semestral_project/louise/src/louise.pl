...
% Uses the Prolog engine and avoids using the dynamic db too much.
	...
	,G = (debug(top_program,'Constructing Top program...',[])
		...
		%  ,Ss_Spec is Ss_Spec
		% ,write(Ss_Spec)
		%  , Ss_Spec is [[m(chain,$1,a,a)-(m(chain,A,B,C):-m(A,D,E),m(B,D,F),m(C,F,E)),m(chain,$1,b,b)-(m(chain,G,H,I):-m(G,J,K),m(H,J,L),m(I,L,K)),m(chain,s,$1,$1)-(m(chain,M,N,O):-m(M,P,Q),m(N,P,R),m(O,R,Q))],
		% [m(chain,$1,a,s)-(m(chain,A,B,C):-m(A,D,E),m(B,D,F),m(C,F,E)),m(chain,s,$1,b)-(m(chain,G,H,I):-m(G,J,K),m(H,J,L),m(I,L,K)),m(chain,s,a,b)-(m(chain,M,N,O):-m(M,P,Q),m(N,P,R),m(O,R,Q))],
		% [m(chain,$1,s,b)-(m(chain,A,B,C):-m(A,D,E),m(B,D,F),m(C,F,E)),m(chain,s,a,$1)-(m(chain,G,H,I):-m(G,J,K),m(H,J,L),m(I,L,K)),m(chain,s,a,b)-(m(chain,M,N,O):-m(M,P,Q),m(N,P,R),m(O,R,Q))],
		% [m(chain,s,a,b)-(m(chain,A,B,C):-m(A,D,E),m(B,D,F),m(C,F,E))]]
	     ,debug_clauses(top_program,'Specialised Top program',Ss_Spec)
		%  >>>>>>
		 ,copy_term(Ss_Spec, MM)
	    %  ,write('***************')
		%  ,print_clauses_m('clauses = ', MM)
		
		,write(MM)
		,store_clauses_to_rename(MM)
		% ,load_renamed_clauses(MM, 'renamed_claused.txt')
		%  <<<<<<
