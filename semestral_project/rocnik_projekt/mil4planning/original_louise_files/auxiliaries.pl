:-module(auxiliaries, [% Experiment auxiliaries
		      ...
		      ,print_clauses/1
			  ,print_clauses_m/2
			  ,store_clauses_to_rename/1
			  ,load_renamed_clauses/2
		      ,program/3
		      ,unifiable_compare/3
		      ,table_program/1
		      ,untable_program/1
...
store_clauses_to_rename(M):-                         
	tell('clauses_to_rename.txt'),
	current_output(Stream),
	print_term(M,[output(Stream)]), told.


load_renamed_clauses(Aisles, Filename) :-
	open(Filename, read, InAisles),
    read(InAisles, Aisles),
    close(InAisles).

% Usage:
% ?- auxiliaries:m_work('$1').
% $1_1
		
% ************************************
% ?- auxiliaries:m_work('$11').
% learn(s/2, _Ps), assert(_Ps). (vypsat do suboru)
%?- auxiliaries:print_clauses().
