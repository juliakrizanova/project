...
:- auxiliaries:set_configuration_option(clause_limit, [3]).
:- auxiliaries:set_configuration_option(max_invented, [1]). %puvodne
% :- auxiliaries:set_configuration_option(max_invented, [2]).
%:- auxiliaries:set_configuration_option(unfold_invented, [true]).
%:- auxiliaries:set_configuration_option(reduction, [none]). subhypothesis ->jedna za druhou; plotkins->zrusi nepouzite klauzule
...
% s([a, b, c, d, a, b, c, d, a, b, c, d, p], []).
% s([a, b, c, d, a, b, c, d, a, b, c, d, p, p], []).
% s([a, b, c, d, a, b, c, d, a, b, c, d, p, p, p], []).
% learn(s/2, _Ps), print_clauses(_Ps), length(_Ps,N).
% ?- list_mil_problem(s/2).
% ?- learn(s/2).

%riadok 155 v louise.pl
%upravit premennu Ss_Spec (je to zoznam) tak, aby kazdy obsahoval ine cislo v dolarovej premennej
%poslat tuto premennu dalej

% s(A,B):-a(A,C),b(C,B).
% s(A,B):-a(A,C),s(C,D),b(D,B).
% true.
% -> skopirovat do suboru
% +background knowledge
% projit pro vsechny pozitivni a negativni priklady a overit, ze dava spravne vysledky


% program :-
%     open('/home/juliakrizanova/Documents/mff_cuni4/anbn_output17.txt', write, Stream),
%     % forall(man(Man), (write(Stream,Man),nl(Stream))),
% 	forall(learn(s/2), (write(Stream, s/2),nl(Stream))),
% 	% write(Stream, learn(s/2))
